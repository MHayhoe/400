...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 10 12:29:05 2019
...
def plot_bet_distributions(t_start, t_end,timestamp):
    plt.figure(4)
    tricks_per_bet = np.zeros((14,14,4))
    for t in range(t_start, t_end):
        for p in range(4):
            tricks_per_bet[int(Bets[t][p]), int(Tricks[t][p]),p] += 1
    for j in range(2,14):
        plt.subplot(4,4,j-1)
        for p in range(4):
            plt.plot(tricks_per_bet[j,:,p])
        plt.plot([j,j],[0,np.max(tricks_per_bet[j,:,:])])
        plt.title('Bet ' + str(j))
    plt.tight_layout()
    plt.show()
...
def plot_action_performance(timestamp,t):
    ...
    bins = range(14);
    # Histogram of tricks
    plt.hist([Tricks[i][0] for i in range(-train_interval/2,0)], bins, alpha=0.5, label='Player 1')
    plt.hist([Tricks[i][1] for i in range(-train_interval/2,0)], bins, alpha=0.5, label='Player 2')
    plt.hist([Tricks[i][2] for i in range(-train_interval/2,0)], bins, alpha=0.5, label='Player 3')
    plt.hist([Tricks[i][3] for i in range(-train_interval/2,0)], bins, alpha=0.5, label='Player 4')
